...
interface BasicsFormProps {
  metadata: StudyMetadata & { lesson_title: string };
  onChange: (field: string, value: string) => void;
  disabled: boolean;
  uiLanguage: UILanguage;
}

export const BasicsForm: React.FC<BasicsFormProps> = ({ metadata, onChange, disabled, uiLanguage }) => {
    ...
    <div className={`bg-white rounded-2xl shadow-sm p-6 border transition-all duration-500 ${isComplete ? 'border-green-100 ring-4 ring-green-50' : 'border-gray-200'}`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold text-gray-900 uppercase tracking-widest">{t.basics.title}</h2>
            <p className="text-[10px] text-gray-400 font-medium">{t.basics.subtitle}</p>
          </div>
        </div>
        {isComplete && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
        ...
        <div className="sm:col-span-2">
            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 ml-1">
                {t.basics.lessonTitle}
            </label>
            <input
                type="text"
                value={metadata.lesson_title}
        ...
        <div>
          ...
          >
            <option value="">{t.basics.select}</option>
            {t.basics.days.map((day, idx) => (
               <option key={idx} value={translations.en.basics.days[idx]}>{day}</option>
            ))}
          </select>
        </div>
      </div>
